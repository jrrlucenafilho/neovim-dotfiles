return {
	...
	config = function()
		-- Modest dropdown size; still wide enough for long model ids (CodeCompanion: `kind = "codecompanion.nvim"`).
		local ui_select_opts = require("telescope.themes").get_dropdown({
			layout_config = {
				width = function(_, max_columns, _)
					return math.floor(max_columns * 0.72)
				end,
				height = function(_, _, max_lines)
					return math.min(max_lines, math.floor(vim.o.lines * 0.45))
				end,
			},
		})
